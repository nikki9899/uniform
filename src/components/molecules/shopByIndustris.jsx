...
const ShopByIndustries = ({ shopByCategory }) => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 lg:gap-x-5 md:gap-x-5 md:gap-y-5 lg:gap-y-9 ">
            ...
            {products.slice(0, isDesktop ? 5 : (showAllProducts ? products.length : 3)).map((product) => (
                <Link key={product.id} href={`/plp/${product.title}`}>
                    <div className="col-span-1 mt-4">
                        <ImageCard
                            iconDisplay="hidden"
                        ...
                        <div className="flex justify-between mt-3">
                            ...
                            <span className=" overflow-hidden text-center text-[7px] sm:text-xs sm:leading-6 leading-4 font-bold lg:text-sm lg:leading-[26.645px] ml-[13rem] xs:ml-[18rem] md:ml-[8rem] lg:ml-[12rem] pr-4">
                                5000
                            </span>
                        </div>
            ...
            {!isDesktop && (
                <div className="md:hidden flex justify-center item-center mt-[30px] h-6 ">
                    <Button variant="primary" rounded="full" onClick={handleSeeAllClick}>
